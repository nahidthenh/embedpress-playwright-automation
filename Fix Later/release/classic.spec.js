...
test('Open Classic Editor and Check YT Video Embed', async ({ page }) => {
    await page.goto('https://ep-automation.mdnahidhasan.com/wp-admin/post.php?post=9610&action=edit&classic-editor');

    // Fill YouTube embed code with a new line in between
    await page.locator('#content').fill('[embedpress]https://youtu.be/x-hxSgtO_Zw[/embedpress]\n[embedpress_pdf]https://ep-automation.mdnahidhasan.com/wp-content/uploads/2024/07/sample_pdf.pdf[/embedpress_pdf]');
...
test('Open Classic Editor and Check Modern PDF Embed', async ({ page }) => {
    ...
    // Fill PDF embed code
    await page.locator('#content').fill('[embedpress_pdf]https://ep-automation.mdnahidhasan.com/wp-content/uploads/2024/07/sample_pdf.pdf[/embedpress_pdf]');
