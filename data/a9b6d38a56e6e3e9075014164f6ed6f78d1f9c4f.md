...
- Name: Embed Gutenberg Voxsnap source
- Location: /home/runner/work/embedpress-playwright-automation/embedpress-playwright-automation/tests/gutenberg/voxsnap.spec.js:3:1
...
Call log:
  ...
  - waiting for locator('iframe').contentFrame().locator('.player_controls')
    ...
    at /home/runner/work/embedpress-playwright-automation/embedpress-playwright-automation/tests/gutenberg/voxsnap.spec.js:5:85
```
...
```ts
  ...
  2 |
  3 | test('Embed Gutenberg Voxsnap source', async ({ page }) => {
  4 |     await page.goto('https://embedpress.wpqa.site/gu-voxsnap/');
> 5 |     await expect(page.locator('iframe').contentFrame().locator('.player_controls')).toBeVisible();
    |                                                                                     ^ Error: Timed out 5000ms waiting for expect(locator).toBeVisible()
  6 | });
  7 | // As now we only have embed support for Voxsnap 
```