# Test info

- Name: Embed Classic WordPress.Tv source
- Location: /home/runner/work/embedpress-playwright-automation/embedpress-playwright-automation/tests/classic/wordpress-tv.spec.js:3:1

# Error details

```
Error: locator.click: Test timeout of 60000ms exceeded.
Call log:
  - waiting for locator('iframe[title="VideoPress Video Player"]').contentFrame().getByText('There’s been an error Something went wrong and we\'re unable to play the video.')

    at /home/runner/work/embedpress-playwright-automation/embedpress-playwright-automation/tests/classic/wordpress-tv.spec.js:5:175
```

# Page snapshot

```yaml
- link "Skip to content":
  - /url: "#main"
- banner:
  - navigation "Primary Navigation":
    - list:
      - listitem:
        - link "Home":
          - /url: https://embedpress.wpqa.site/
      - listitem:
        - link "Random Gutenberg":
          - /url: https://embedpress.wpqa.site/random-gutenberg/
      - listitem:
        - link "Random Elementor":
          - /url: https://embedpress.wpqa.site/random-elementor/
      - listitem:
        - link "Random Classic":
          - /url: https://embedpress.wpqa.site/random-classic/
      - listitem:
        - link "Automation Report":
          - /url: https://nahidthenh.github.io/embedpress-playwright-automation/
      - listitem:
        - link "Md. Nahid Hasan (Admin)":
          - /url: https://embedpress.wpqa.site/wp-admin
  - paragraph
  - search "Search form":
    - searchbox "Search input"
    - textbox "Search autocomplete" [disabled]
  - button "Search magnifier":
    - img
  - paragraph
- heading "Classic WordPress Tv" [level=1]
- main:
  - article
```

# Test source

```ts
  1 | const { test, expect } = require('@playwright/test');
  2 |
  3 | test('Embed Classic WordPress.Tv source', async ({ page }) => {
  4 |     await page.goto('playwright-classic-editor/classic-wordpress-tv/');
> 5 |     await page.locator('iframe[title="VideoPress Video Player"]').contentFrame().getByText('There’s been an error Something went wrong and we\'re unable to play the video.').click();
    |                                                                                                                                                                               ^ Error: locator.click: Test timeout of 60000ms exceeded.
  6 | });
  7 | // As now we only have embed support for WordPress.Tv
  8 |
  9 | // Note: Some how now wordpress tv player not support on chromium browser 
```